...
public struct PlayerState<Value>: DynamicProperty {
    ...
    private let get: @MainActor (RunPlayer) -> Value

    private let set: @MainActor (RunPlayer, Value) -> Void
    ...
    ///
    public init(_ path: KeyPath<RunPlayer, Value>) {
        get = { $0[keyPath: path] }
        set = { _, _ in }
    }
    ...
    ///
    public init(_ path: ReferenceWritableKeyPath<RunPlayer, Value>) {
        get = { $0[keyPath: path] }
